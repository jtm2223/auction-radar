...
class NHStateSurplusScraper(BaseScraper):
    """Scraper for New Hampshire municipal surplus auctions via Municibid."""
    
    def __init__(self):
        super().__init__('nh_state_surplus', 'https://municibid.com')
    
    def crawl(self) -> List[Dict[str, Any]]:
        """Crawl New Hampshire municipal surplus auctions from Municibid."""
        lots = []
        
        try:
            # Fetch New Hampshire municipal auctions from Municibid
            url = "https://municibid.com/Browse/R3777826/New_Hampshire"
            response = self.safe_get(url)
            
            if not response:
                logger.warning("Could not fetch NH Municibid page")
                return []
            ...
            logger.debug(f"Found {len(auction_links)} auction links on NH page")
            ...
            logger.debug(f"Found {len(all_items)} potential NH auction items")
                ...
                try:
                    ...
                    if lot_data:
                        ...
                        vehicle_keywords = ['FORD', 'CHEVY', 'GMC', 'TOYOTA', 'HONDA', 'NISSAN', 'LEXUS', 
                                          ...
                                          'CRUISER', 'PICKUP', 'VEHICLE', 'AUTO', 'SEDAN', 'COUPE', 
                                          'WAGON', 'JEEP', 'DODGE', 'CHRYSLER', '4RUNNER', 'LAND CRUISER', 
                                          'TACOMA', 'TUNDRA', 'FRONTIER', 'TITAN', 'CAMPER', 'TRAILER', 
                                          'RV', 'MOTORHOME']
                        if any(keyword in text for keyword in vehicle_keywords):
                            lots.append(lot_data)
                except Exception as e:
                    logger.debug(f"Error parsing NH auction item: {e}")
                    continue
                    
            logger.info(f"Found {len(lots)} New Hampshire municipal surplus lots")
                    
        except Exception as e:
            logger.error(f"Error fetching NH Municibid data: {e}")
        
        return lots
    
    def _parse_municibid_item(self, item, href=None) -> Dict[str, Any]:
        """Parse a Municibid auction item for New Hampshire."""
        try:
            ...
            if href:
                ...
                if any(word in url_desc.upper() for word in ['FORD', 'CHEVY', 'GMC', 'TOYOTA', 'HONDA', 'NISSAN', 'TRUCK', 'CAR', 'VAN', 'POLICE', 'FIRE']):
                    vehicle_desc = url_desc.replace('_', ' ')
            
            # If not found in URL, check text lines
            if not vehicle_desc:
                for line in lines:
                    if any(word in line.upper() for word in ['FORD', 'CHEVY', 'GMC', 'TOYOTA', 'HONDA', 'NISSAN', 'TRUCK', 'CAR', 'VAN', 'POLICE', 'FIRE']):
                        vehicle_desc = line
                        break
            ...
            # Extract location - New Hampshire cities
            city = "Concord"  # Default to state capital
            nh_cities = ['Manchester', 'Nashua', 'Concord', 'Derry', 'Dover', 'Rochester', 'Salem', 
                        'Merrimack', 'Hudson', 'Londonderry', 'Keene', 'Portsmouth', 'Goffstown', 
                        'Laconia', 'Hampton', 'Milford', 'Durham', 'Exeter', 'Hanover', 'Lebanon']
            
            for nh_city in nh_cities:
                if nh_city.upper() in text_content.upper():
                    city = nh_city
                    break
            ...
            lot_id = f"nh_municibid_{auction_id}" if auction_id else f"nh_municibid_{hash(vehicle_desc[:50]) % 100000}"
            ...
            lot_data = {
                ...
                'location_name': 'New Hampshire Municipal Surplus',
                'location_city': city,
                'location_state': 'NH',
                'year': year,
                'make': make,
            ...
            logger.debug(f"Error parsing NH Municibid item: {e}")
            return None
    ...
    def _extract_vehicle_info(self, description: str) -> tuple:
        ...
        patterns = [
            r'(\d{4})\s+([A-Za-z]+(?:\s+[A-Za-z]+)?)\s+([A-Za-z0-9\s\-]+)',  # 2015 Ford F550
            r'([A-Za-z]+(?:\s+[A-Za-z]+)?)\s+([A-Za-z0-9\s\-]+)\s+(\d{4})',  # Ford F550 2015
            r'(\d{4})\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s+(.+)',            # 2006 Ford Econoline Van
        ]
